...
| ディレクトリ | 役割 |
|--------------|------|
| `Defines/` | クロスプラットフォームの型定義。`builtin.xml` は `vec2`/`vec3`/`vec4` などの bean を定義し、ターゲットごとにマッピングされます — クライアントは `UnityEngine.Vector*`、サーバーは `System.Numerics.Vector*`。 |
| `Excels/` | Excel ソースのルート。`gameframex` インポーターが再帰的にスキャンします（下記「Excel ソースレイアウト」を参照）。 |
| `Tools/` | Luban ランタイム: `Luban.dll` と依存アセンブリ、`Templates/` コードテンプレート。gen スクリプトは `dotnet Tools/Luban.dll` で起動します。 |
...
1 つの論理テーブルのデータは複数の Excel ファイルに分散できます — テーブルが大きくなったときや、行をモジュール別にまとめたいときに便利です。インポーターは `{エクスポートテーブル名}` が同一のファイルをすべて 1 つのテーブルにマージし、エクスポート時に行が連結されます。

命名: 各シャードで `{ソート番号}` と `{エクスポートテーブル名}` を同じにし、`{中国語ラベル}` だけを変えて区別します。ラベル（番号や分類を含む）はドキュメント専用で、インポーターは解析しません。

| 分割ファイル | マージ後のテーブル | 典型的な用途 |
|--------------|--------------------|--------------|
| `L-Localization-成就.xlsx`、`L-Localization-文本.xlsx`、`L-Localization-UI.xlsx` | `TbLocalization` | モジュール別にローカライズテキストを整理 |
| `D-ItemConfig-道具表-道具-1001.xlsx`、`D-ItemConfig-道具表-道具-1002.xlsx`、… | `TbItemConfig` | 大きなテーブルを管理しやすいファイル群に分割 |
